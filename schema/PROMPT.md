# Generar una rutina con Claude

Copia **todo lo que hay debajo de la linea**, pegalo en una conversacion nueva con
Claude, rellena el apartado final y pega el JSON resultante en la app:
**Rutinas → Importar**.

Este archivo se mantiene sincronizado con `src/data/prompt-template.ts`, que es lo que
copia el boton "Copiar prompt" de la aplicacion. El contrato formal esta en
[`routine.v1.schema.json`](routine.v1.schema.json).

---

Necesito que me generes una rutina de entrenamiento en JSON para mi aplicacion Training Lab.
